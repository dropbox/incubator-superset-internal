...
var _extends = require("@babel/runtime/helpers/extends"), _objectWithoutPropertiesLoose = require("@babel/runtime/helpers/objectWithoutPropertiesLoose"), _createForOfIteratorHelperLoose = require("@babel/runtime/helpers/createForOfIteratorHelperLoose"), prettify = require("@emotion/css-prettifier");
...
var _extends__default = _interopDefault(_extends), _objectWithoutPropertiesLoose__default = _interopDefault(_objectWithoutPropertiesLoose), _createForOfIteratorHelperLoose__default = _interopDefault(_createForOfIteratorHelperLoose), prettify__default = _interopDefault(prettify);
...
}, tickledCssProps = new WeakMap, getTickledClassName = function(cssProp) {
  return tickledCssProps.get(cssProp);
}, tickle = function(wrapper) {
  var isShallow = "function" == typeof wrapper.dive;
  return wrapper.find("EmotionCssPropInternal").forEach((function(el) {
    var cssProp = el.props().css;
    if (cssProp) {
      var wrapped = (isShallow ? el.dive() : el.children()).first();
...
function getNodes(node, nodes) {
  if (void 0 === nodes && (nodes = []), Array.isArray(node)) {
    for (var _step, _iterator = _createForOfIteratorHelperLoose__default.default(node); !(_step = _iterator()).done; ) {
      getNodes(_step.value, nodes);
    }
    return nodes;
  }
  if ("object" == typeof node && nodes.push(node), node.children) for (var _step2, _iterator2 = _createForOfIteratorHelperLoose__default.default(node.children); !(_step2 = _iterator2()).done; ) {
...
function getLabelsFromClassName(keys, className) {
  return flatMap(className.split(" "), (function(cls) {
    var _cls$split = cls.split("-"), key = _cls$split[0], labels = (_cls$split[1], _cls$split.slice(2));
    return keys.includes(key) ? labels : null;
  })).filter(Boolean);
}

function isShallowEnzymeElement(element, keys, labels) {
  return !(element.children || []).map((function(_ref) {
    var _ref$props = _ref.props;
    return (void 0 === _ref$props ? {} : _ref$props).className || "";
  })).filter(Boolean).some((function(className) {
    return getLabelsFromClassName(keys, className).every((function(childLabel) {
...
var createConvertEmotionElements = function(keys, printer) {
  return function(node) {
    ...
    if (isEmotionCssPropEnzymeElement(node)) {
      var _className = getTickledClassName(node.props.css), labels = getLabelsFromClassName(keys, _className || "");
      if (isShallowEnzymeElement(node, keys, labels)) {
        var emotionType = node.props.__EMOTION_TYPE_PLEASE_DO_NOT_USE__, type = "string" == typeof emotionType ? emotionType : emotionType.displayName || emotionType.name || "Component";
...
function clean(node, classNames) {
  if (Array.isArray(node)) for (var _step3, _iterator3 = _createForOfIteratorHelperLoose__default.default(node); !(_step3 = _iterator3()).done; ) {
    clean(_step3.value, classNames);
  } else {
    if (node.children) for (var _step4, _iterator4 = _createForOfIteratorHelperLoose__default.default(node.children); !(_step4 = _iterator4()).done; ) {
