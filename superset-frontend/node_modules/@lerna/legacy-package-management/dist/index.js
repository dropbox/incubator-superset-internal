...
var require_types = __commonJS({
  "libs/core/src/lib/npm-conf/types.ts"(exports) {
    ...
    exports.types = {
      ...
      cafile: import_path16.default,
      cache: import_path16.default,
      ...
      global: Boolean,
      globalconfig: import_path16.default,
      ...
      "init-module": import_path16.default,
      ...
      tag: String,
      timing: Boolean,
      tmp: import_path16.default,
      unicode: Boolean,
      "unsafe-perm": Boolean,
      "update-notifier": Boolean,
      usage: Boolean,
      user: [Number, String],
      userconfig: import_path16.default,
      umask: Umask,
      version: Boolean,
...
var require_defaults = __commonJS({
  "libs/core/src/lib/npm-conf/defaults.ts"(exports) {
    ...
    if (home) {
      ...
      home = import_path16.default.resolve(temp, `npm-${uidOrPid}`);
    }
    var cacheExtra = process.platform === "win32" ? "npm-cache" : ".npm";
    var cacheRoot = process.platform === "win32" && process.env["APPDATA"] || home;
    var cache = import_path16.default.resolve(cacheRoot, cacheExtra);
    var defaults;
    var globalPrefix;
    Object.defineProperty(exports, "defaults", {
      get() {
        ...
        if (process.env["PREFIX"]) {
          ...
          globalPrefix = import_path16.default.dirname(process.execPath);
        } else {
          globalPrefix = import_path16.default.dirname(import_path16.default.dirname(process.execPath));
          if (process.env["DESTDIR"]) {
            globalPrefix = import_path16.default.join(process.env["DESTDIR"], globalPrefix);
          }
        }
        defaults = {
          access: null,
          ...
          "commit-hooks": true,
          global: false,
          globalconfig: import_path16.default.resolve(globalPrefix, "etc", "npmrc"),
          "global-style": false,
          ...
          "init-module": import_path16.default.resolve(home, ".npm-init.js"),
          "init-author-name": "",
          "init-author-email": "",
          "init-author-url": "",
          "init-version": "1.0.0",
          ...
          userconfig: import_path16.default.resolve(home, ".npmrc"),
...
var require_npm_conf = __commonJS({
  "libs/core/src/lib/npm-conf/index.ts"(exports, module2) {
    ...
    function npmConf3(opts) {
      ...
      conf.addEnv();
      conf.loadPrefix();
      const projectConf = import_path16.default.resolve(conf["localPrefix"], ".npmrc");
      ...
      if (conf["get"]("prefix")) {
        const etc = import_path16.default.resolve(conf["get"]("prefix"), "etc");
        conf.root.globalconfig = import_path16.default.resolve(etc, "npmrc");
        conf.root.globalignorefile = import_path16.default.resolve(etc, "npmignore");
      }
...
function printCommandBanner(id, event, cmd, path16) {
  return console.log(`
> ${id ? `${id} ` : ""}${event} ${path16}
> ${cmd.trim().replace(/\n/g, "\n> ")}
`);
  ...
  return Promise.all([Package.lazy(srcPackageRef), Package.lazy(destPackageRef)]).then(
    ...
    ([srcPackage, destPackage]) => {
      const actions = Object.keys(srcPackage.bin).map((name) => {
        const srcLocation = srcPackage.contents ? import_path13.default.resolve(srcPackage.location, srcPackage.contents) : srcPackage.location;
        const src = import_path13.default.join(srcLocation, srcPackage.bin[name]);
        const dst = import_path13.default.join(destPackage.binLocation, name);
        return { src, dst };
      });
      if (actions.length === 0) {
        return Promise.resolve();
...
var import_fs_extra8, import_p_map3, import_path13;
...
function resolveSymbolicLink(filePath) {
  ...
  const resolvedPath = lstat.isSymbolicLink() ? import_path14.default.resolve(import_path14.default.dirname(filePath), import_fs_extra9.default.readlinkSync(filePath)) : false;
  return {
    resolvedPath,
    lstat
  };
...
function resolveWindowsSymlink(filePath) {
  ...
  if (lstat.isFile() && !resolvedPath) {
    try {
      return import_path14.default.resolve(import_path14.default.dirname(filePath), readCmdShim.sync(filePath));
    } catch (e) {
      return false;
    }
  }
  return resolvedPath && import_path14.default.resolve(resolvedPath);
}
var import_fs_extra9, import_npmlog36, import_path14, readCmdShim;
...
function symlinkDependencies(packages, packageGraph, tracker) {
  ...
  return (0, import_p_map_series.default)(nodes, (currentNode) => {
    ...
    return (0, import_p_map4.default)(currentNode?.localDependencies ?? /* @__PURE__ */ new Map(), ([dependencyName, resolved]) => {
      if (resolved.type === "directory") {
        return;
      }
      const dependencyNode = packageGraph?.get(dependencyName);
      const targetDirectory = import_path15.default.join(currentNodeModules ?? "", dependencyName);
      let chain = Promise.resolve();
      ...
      chain = chain.then((dirExists) => {
        if (dirExists) {
          ...
          return import_fs_extra10.default.ensureDir(import_path15.default.dirname(targetDirectory));
        }
      });
      const dependencyLocation = dependencyNode?.pkg.contents ? import_path15.default.resolve(dependencyNode.location, dependencyNode.pkg.contents) : dependencyNode?.location;
...
var import_fs_extra10, import_p_map4, import_p_map_series, import_path15;
  ...
  "libs/commands/bootstrap/src/index.ts"(exports, module2) {
    ...
    var BootstrapCommand = class extends Command {
      ...
      hoistedDirectory(dependency) {
        return import_path16.default.join(this.project.rootPath, "node_modules", dependency);
      }
      hoistedPackageJson(dependency) {
        try {
          return require(import_path16.default.join(this.hoistedDirectory(dependency), "package.json"));
        } catch (e) {
          return {};
        }
      }
        ...
        if (rootSet.size) {
          ...
          actions.push(() => {
            const candidates = root.filter((dep) => dep.dependents.length).reduce((list2, { name, dependents }) => {
              const dirs = dependents.filter((pkg) => pkg.nodeModulesLocation !== rootPkg.nodeModulesLocation).map((pkg) => import_path16.default.join(pkg.nodeModulesLocation, name));
  ...
  "libs/commands/add/src/lib/get-range-to-reference.ts"(exports, module2) {
    ...
    function getRangeToReference(spec, deps, loc, prefix) {
      ...
      if (spec.saveRelativeFileSpec) {
        return import_npm_package_arg7.default.resolve(spec.name, import_path16.default.relative(loc, spec.version), loc).saveSpec;
      }
  ...
  "libs/commands/add/src/index.ts"(exports, module2) {
    ...
    var AddCommand = class extends Command {
      ...
      initialize() {
        ...
        this.dirs = new Set(this.options.globs.map((fp) => import_path16.default.resolve(this.project.rootPath, fp)));
  ...
  "libs/commands/link/src/index.ts"(exports, module2) {
    ...
    var LinkCommand = class extends Command {
      ...
      initialize() {
        ...
        this.targetGraph = this.options.forceLocal ? new PackageGraph(this.allPackages, "allDependencies", true) : this.packageGraph;
      }
      execute() {
        ...
        if (this.targetGraph) {
          for (const targetNode of this.targetGraph.values()) {
            ...
            for (const depNode of targetNode.localDependents.values()) {
              const depVersion = (0, import_slash2.default)(import_path16.default.relative(depNode.pkg.location, targetNode.pkg.location));
...
// packages/legacy-package-management/src/index.ts
var src_exports = {};
...
var linkCmd = __toESM(require_command3());
