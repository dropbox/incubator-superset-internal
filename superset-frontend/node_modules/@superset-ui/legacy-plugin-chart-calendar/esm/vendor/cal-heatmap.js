...
var CalHeatMap = function () {
  ...
  var self = this;
  self.tip = d3tip().attr('class', 'd3-tip').direction('n').offset([-5, 0]).html(d => `
      ${self.options.timeFormatter(d.t)}: <strong>${self.options.valueFormatter(d.v)}</strong>
    `);
  self.legendTip = d3tip().attr('class', 'd3-tip').direction('n').offset([-5, 0]).html(d => self.options.valueFormatter(d));
