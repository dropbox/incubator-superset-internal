...
function ForceDirected(element, props) {
  ...
  const force = d3.layout.force().nodes(d3.values(nodes)).links(links).size([width, height]).linkDistance(linkLength).charge(charge).on('tick', tick).start();
  ...
  const edgeScale = d3.scale.linear().range([0.1, 0.5]); // add the links and the arrows
