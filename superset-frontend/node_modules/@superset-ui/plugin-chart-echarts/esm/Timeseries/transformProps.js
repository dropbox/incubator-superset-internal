...
export default function transformProps(chartProps) {
  ...
  const padding = getPadding(showLegend, legendOrientation, addYAxisLabelOffset, zoomable, null, addXAxisLabelOffset, yAxisTitlePosition, yAxisTitleMargin, xAxisTitleMargin);
  const legendData = rawSeries.filter(entry => extractForecastSeriesContext(entry.name || '').type === ForecastSeriesEnum.Observation).map(entry => entry.name || '').concat(extractAnnotationLabels(annotationLayers, annotationData));
  const echartOptions = {
    useUTC: true,
    grid: { ...defaultGrid,
      ...padding
    ...
    tooltip: { ...defaultTooltip,
      ...
      formatter: params => {
        ...
        Object.keys(prophetValues).forEach(key => {
          ...
          const content = formatProphetTooltipSeries({ ...value,
            seriesName: key,
            formatter
          });
