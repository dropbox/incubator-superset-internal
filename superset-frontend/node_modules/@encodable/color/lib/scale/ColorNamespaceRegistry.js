...
var _ColorNamespace = _interopRequireDefault(require("./ColorNamespace"));
...
class ColorNamespaceRegistry {
  constructor({
    ...
    this.namespaceStates = new _registry.SyncRegistry({
      name,
      globalId,
      ...rest
    }); // the instances are always local since these are wrappers

    this.namespaceInstances = new _registry.SyncRegistry({
      name,
      ...rest
    });
  ...
  get(namespace = this.getDefaultNamespace()) {
    if (!this.namespaceStates.has(namespace)) {
      const ns = new _ColorNamespace.default(namespace);
    ...
    const ns = new _ColorNamespace.default(this.namespaceStates.get(namespace));
    this.namespaceInstances.registerValue(namespace, ns);
    return ns;
  }
...
exports.default = ColorNamespaceRegistry;