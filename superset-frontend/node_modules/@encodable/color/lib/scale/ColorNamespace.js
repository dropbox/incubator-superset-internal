...
class ColorNamespace {
  ...
  getScale(scheme) {
    const schemeName = scheme ?? (0, _scheme.getColorSchemeRegistry)().categorical.getDefaultKey() ?? 'undefined';

    if (typeof this.state.scales[schemeName] === 'undefined') {
      // create scale
      const scale = new _ScaleCategoricalColor.default((0, _scheme.getCategoricalScheme)(schemeName)?.colors.slice() ?? [], this.state.manualColors); // add state to lookup
