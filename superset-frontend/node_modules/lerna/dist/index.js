  ...
  "libs/core/src/lib/package.ts"() {
    ...
    Package = class {
      ...
      constructor(pkg2, location, rootPath = location) {
        const resolved = import_npm_package_arg2.default.resolve(pkg2.name, `file:${import_path2.default.relative(rootPath, location)}`, rootPath);
        this.name = pkg2.name;
        this[PKG2] = pkg2;
        ...
        const explorer = (0, import_cosmiconfig.cosmiconfigSync)("lerna", {
          loaders: {
            ...
            ".json": (filepath, content) => {
              ...
              try {
                return (0, import_devkit.parseJson)(content);
              } catch (err) {
    ...
    return new Promise((resolve, reject) => {
      (0, import_conventional_recommended_bump.default)(options, (err, data) => {
        ...
        if (prereleaseId) {
          ...
          import_npmlog16.default.verbose(type, "increment %s by %s", pkg2.version, prereleaseType);
          resolve(applyBuildMetadata(import_semver3.default.inc(pkg2.version, prereleaseType, prereleaseId), buildMetadata));
        } else {
          ...
          import_npmlog16.default.verbose(type, "increment %s by %s", pkg2.version, releaseType);
          resolve(applyBuildMetadata(import_semver3.default.inc(pkg2.version, releaseType), buildMetadata));
        }
      });
    });
  });
...
function readExistingChangelog(pkg2) {
  const changelogFileLoc = import_path8.default.join(pkg2.location, "CHANGELOG.md");
  let chain = Promise.resolve();
...
function updateChangelog(pkg2, type, { changelogPreset, rootPath, tagPrefix = "v", version }) {
  import_npmlog17.default.silly(type, "for %s at %s", pkg2.name, pkg2.location);
...
function filterResultList(pkgList, viewOptions) {
  let result = viewOptions.showAll ? pkgList.slice() : pkgList.filter((pkg2) => !pkg2.private);
...
function formatParseable(resultList, viewOptions) {
  return resultList.map(viewOptions.showLong ? makeParseable : (pkg2) => pkg2.location).join("\n");
}
...
function formatColumns(resultList, viewOptions) {
  const formattedResults = resultList.map((result) => {
    ...
    formatted.location = import_chalk2.default.grey(import_path9.default.relative(".", result.location));
    return formatted;
  });
...
function printCommandBanner(id, event, cmd, path18) {
  return console.log(`
> ${id ? `${id} ` : ""}${event} ${path18}
> ${cmd.trim().replace(/\n/g, "\n> ")}
`);
  ...
  if (!dryRun) {
    chain = chain.then(() => {
      ...
      if (pkg2.contents !== pkg2.location) {
        manifestLocation = import_path13.default.join(pkg2.contents, "package.json");
      }
...
async function tempWrite(fileContent, filePath) {
  ...
  await (0, import_make_dir.default)(import_path14.default.dirname(tempPath));
  await write(tempPath, fileContent);
  return tempPath;
}
var import_graceful_fs, import_is_stream, import_make_dir, import_path14, import_temp_dir, import_util2, uuid, writeFileP, tempfile, writeStream, temp_write_default;
var init_temp_write = __esm({
  "libs/core/src/lib/temp-write.ts"() {
    ...
    tempfile = (filePath) => import_path14.default.join(import_temp_dir.default, uuid.v4(), filePath || "");
    ...
    tempWrite.sync = (fileContent, filePath) => {
      const tempPath = tempfile(filePath);
      import_make_dir.default.sync(import_path14.default.dirname(tempPath));
...
function packDirectory(_pkg, dir, options) {
  ...
  opts.log.verbose("pack-directory", import_path16.default.relative(".", pkg2.contents));
  let chain = Promise.resolve();
...
var import_path16, import_npm_packlist, import_npmlog29, import_tar2;
...
function runTopologically(packages, runner, { concurrency, graphType, rejectCycles } = {}) {
  ...
  return new Promise((resolve, reject) => {
    const returnValues = [];
    const queueNextAvailablePackages = () => graph.getAvailablePackages().forEach(({ pkg: pkg2, name }) => {
      graph.markAsTaken(name);
      queue2.add(
        () => runner(pkg2).then((value) => returnValues.push(value)).then(() => graph.markAsDone(pkg2)).then(() => queueNextAvailablePackages())
      ).catch(reject);
    });
...
var import_npmlog33, import_rest, import_git_url_parse, childProcess9;
    ...
    var CleanCommand = class extends Command {
      ...
      initialize() {
        ...
        return chain.then(() => {
          ...
          this.logger.info(
            "clean",
            this.directoriesToDelete.map((dir) => import_path18.default.relative(this.project.rootPath, dir)).join("\n")
          );
  ...
  "libs/commands/exec/src/index.ts"(exports2, module2) {
    ...
    var ExecCommand = class extends Command {
      ...
      runCommandInPackagesParallel() {
        return (0, import_p_map5.default)(this.filteredPackages, (pkg2) => this.runCommandInPackageStreaming(pkg2));
      }
      ...
      runCommandInPackageStreaming(pkg2) {
        return childProcess10.spawnStreaming(this.command, this.args, this.getOpts(pkg2), this.prefix && pkg2.name);
      }
      runCommandInPackageCapturing(pkg2) {
        return childProcess10.spawn(this.command, this.args, this.getOpts(pkg2));
      }
    };
  ...
  "libs/commands/import/src/index.ts"(exports2, module2) {
    ...
    var ImportCommand = class extends Command {
      ...
      initialize() {
        ...
        const externalRepoPath = import_path18.default.resolve(inputPath);
        const externalRepoBase = import_path18.default.basename(externalRepoPath);
        ...
        const packageJson = import_path18.default.join(externalRepoPath, "package.json");
        ...
        const targetDir = import_path18.default.join(targetBase, externalRepoBase);
        const gitRepoRoot = this.getWorkspaceRoot();
        const lernaRootRelativeToGitRoot = import_path18.default.relative(gitRepoRoot, this.project.rootPath);
        this.targetDirRelativeToGitRoot = import_path18.default.join(lernaRootRelativeToGitRoot, targetDir);
        if (import_fs_extra11.default.existsSync(import_path18.default.resolve(this.project.rootPath, targetDir))) {
      ...
      getPackageDirectories() {
        return this.project.packageConfigs.filter((p) => p.endsWith("*")).map((p) => import_path18.default.dirname(p));
      }
      getTargetBase() {
    ...
    var InitCommand = class extends Command {
      ...
      initialize() {
        ...
        if (!this.gitInitialized()) {
          ...
          return childProcess10.exec("git", ["init"], this.execOpts);
        }
      }
      execute() {
        let chain = Promise.resolve();
      ...
      ensureGitIgnore() {
        const gitIgnorePath = import_path18.default.join(this.project.rootPath, ".gitignore");
        let chain = Promise.resolve();
      ...
      ensureConfig() {
        ...
        if (!hasExistingPackageJson) {
          ...
          chain = chain.then(() => {
            ...
            if (useWorkspaces) {
              pkg2.workspaces = [Project.PACKAGE_GLOB];
            }
            return (0, import_devkit2.writeJsonFile)(import_path18.default.join(this.project.rootPath, "package.json"), pkg2, {
              spaces: 2
            });
          });
        } else {
...
var require_git_add = __commonJS({
  "libs/commands/version/src/lib/git-add.ts"(exports2, module2) {
    ...
    function resolvePrettier() {
      if (!resolvedPrettier) {
        try {
          const packageJson = (0, import_devkit2.readJsonFile)(import_path18.default.join(import_devkit2.workspaceRoot, "package.json"));
          const hasPrettier = packageJson.devDependencies?.prettier || packageJson.dependencies?.prettier;
          if (!hasPrettier) {
            return;
          }
          const prettierPath = import_path18.default.join(import_devkit2.workspaceRoot, "node_modules", "prettier");
    ...
    function maybeFormatFile(filePath) {
      ...
      const ignorePath = import_path18.default.join(import_devkit2.workspaceRoot, ".prettierignore");
      const fullFilePath = import_path18.default.join(import_devkit2.workspaceRoot, filePath);
...
var require_git_push = __commonJS({
  "libs/commands/version/src/lib/git-push.ts"(exports2, module2) {
    ...
    function gitPush(remote, branch, opts) {
      ...
      return childProcess10.exec("git", ["push", "--follow-tags", "--no-verify", "--atomic", remote, branch], opts).catch((error) => {
        if (/atomic/.test(error.stderr) || process.env.GIT_REDIRECT_STDERR === "2>&1" && /atomic/.test(error.stdout)) {
          ...
          return childProcess10.exec("git", ["push", "--follow-tags", "--no-verify", remote, branch], opts);
        }
        throw error;
      });
    }
...
var require_git_tag = __commonJS({
  "libs/commands/version/src/lib/git-tag.ts"(exports2, module2) {
    ...
    function gitTag(tag, { forceGitTag, signGitTag }, opts, command = "git tag %s -m %s") {
      ...
      return childProcess10.exec(cmd, interpolatedArgs, opts);
    }
  }
});
...
var require_is_behind_upstream = __commonJS({
  "libs/commands/version/src/lib/is-behind-upstream.ts"(exports2, module2) {
    ...
    function updateRemote(opts) {
      childProcess10.execSync("git", ["remote", "update"], opts);
    }
    function countLeftRight(symmetricDifference, opts) {
      const stdout = childProcess10.execSync(
        "git",
...
var require_remote_branch_exists = __commonJS({
  "libs/commands/version/src/lib/remote-branch-exists.ts"(exports2, module2) {
    ...
    function remoteBranchExists(gitRemote, branch, opts) {
      ...
      const remoteBranch = `${gitRemote}/${branch}`;
      try {
        childProcess10.execSync("git", ["show-ref", "--verify", `refs/remotes/${remoteBranch}`], opts);
        return true;
      } catch (e) {
        return false;
      }
...
var require_is_anything_committed = __commonJS({
  "libs/commands/version/src/lib/is-anything-committed.ts"(exports2, module2) {
    ...
    function isAnythingCommitted(opts) {
      ...
      const anyCommits = childProcess10.execSync("git", ["rev-list", "--count", "--all", "--max-count=1"], opts);
...
var require_update_lockfile_version = __commonJS({
  "libs/commands/version/src/lib/update-lockfile-version.ts"(exports2, module2) {
    ...
    function updateLockfileVersion(pkg2) {
      const lockfilePath = import_path18.default.join(pkg2.location, "package-lock.json");
      let chain = Promise.resolve();
      chain = chain.then(
        () => (0, import_load_json_file3.default)(lockfilePath).catch(() => {
          import_npmlog38.default.verbose("version", `${pkg2.name} has no lockfile. Skipping lockfile update.`);
        })
      );
  ...
  "libs/commands/version/src/index.ts"(exports2, module2) {
    ...
    var VersionCommand = class extends Command {
      ...
      getVersionsForUpdates() {
        ...
        if (repoVersion) {
          ...
          predicate = (node) => applyBuildMetadata(
            import_semver5.default.inc(node.version, increment, resolvePrereleaseId(node.prereleaseId)),
          ...
          const nextVersion = applyBuildMetadata(
            import_semver5.default.inc(this.project.version, increment, resolvePrereleaseId(prereleaseId)),
            this.options.buildMetadata
          );
      ...
      getPrereleasePackageNames() {
        ...
        return collectPackages(this.packageGraph, { isCandidate }).map((pkg2) => pkg2.name);
      }
      ...
      confirmVersions() {
        const changes = this.packagesToVersion.map((pkg2) => {
          let line = ` - ${pkg2.name}: ${pkg2.version} => ${this.updatesVersions.get(pkg2.name)}`;
          if (pkg2.private) {
      ...
      updatePackageVersions() {
        ...
        const actions = [
          (pkg2) => this.runPackageLifecycle(pkg2, "preversion").then(() => pkg2),
          // manifest may be mutated by any previous lifecycle
          (pkg2) => pkg2.refresh(),
          (pkg2) => {
            pkg2.set("version", this.updatesVersions.get(pkg2.name));
            for (const [depName, resolved] of this.packageGraph.get(pkg2.name).localDependencies) {
              const depVersion = this.updatesVersions.get(depName);
              if (depVersion && resolved.type !== "directory") {
                pkg2.updateLocalDependency(resolved, depVersion, this.savePrefix);
              }
            }
          ...
          (pkg2) => this.runPackageLifecycle(pkg2, "version").then(() => pkg2)
        ];
        if (conventionalCommits && changelog) {
          const type = independentVersions ? "independent" : "fixed";
          actions.push(
            (pkg2) => updateChangelog(pkg2, type, {
              changelogPreset,
              rootPath,
              tagPrefix: this.tagPrefix
        ...
        if (this.options.npmClient === "pnpm") {
          chain = chain.then(() => {
            ...
            return childProcess10.exec("pnpm", ["install", "--lockfile-only", "--ignore-scripts", ...npmClientArgs], this.execOpts).then(() => {
        ...
        if (this.options.npmClient === "yarn") {
          chain = chain.then(() => childProcess10.execSync("yarn", ["--version"], this.execOpts)).then((yarnVersion) => {
            this.logger.verbose("version", `Detected yarn version ${yarnVersion}`);
            if (import_semver5.default.gte(yarnVersion, "2.0.0")) {
              this.logger.verbose("version", "Updating root yarn.lock");
              return childProcess10.exec("yarn", ["install", "--mode", "update-lockfile", ...npmClientArgs], {
                ...this.execOpts,
                env: {
                  ...process.env,
      ...
      commitAndTagUpdates() {
        ...
        chain = chain.then(
          () => (0, import_p_map5.default)(this.packagesToVersion, (pkg2) => this.runPackageLifecycle(pkg2, "postversion"))
        );
        if (!this.hasRootedLeaf) {
      ...
      gitCommitAndTagVersionForUpdates() {
        const tags = this.packagesToVersion.map((pkg2) => `${pkg2.name}@${this.updatesVersions.get(pkg2.name)}`);
  ...
  "libs/commands/publish/src/lib/create-temp-licenses.ts"(exports2, module2) {
    ...
    function createTempLicenses(srcLicensePath, packagesToBeLicensed) {
      ...
      packagesToBeLicensed.forEach((pkg2) => {
        pkg2.licensePath = import_path18.default.join(pkg2.contents, licenseFileName);
      });
      return (0, import_p_map5.default)(packagesToBeLicensed, (pkg2) => import_fs_extra11.default.copy(srcLicensePath, pkg2.licensePath, options));
    }
  }
});
...
var require_get_current_tags = __commonJS({
  "libs/commands/publish/src/lib/get-current-tags.ts"(exports2, module2) {
    ...
    function getCurrentTags(execOpts, matchingPattern) {
      ...
      return childProcess10.exec("git", ["tag", "--sort", "version:refname", "--points-at", "HEAD", "--list", matchingPattern], opts).then((result) => {
...
var require_get_unpublished_packages = __commonJS({
  "libs/commands/publish/src/lib/get-unpublished-packages.ts"(exports2, module2) {
    ...
    function getUnpublishedPackages(packageGraph, opts) {
      ...
      const mapper = (pkg2) => import_pacote.default.packument(pkg2.name, opts).then(
        (packument) => {
        ...
        () => {
          import_npmlog38.default.warn("", "Unable to determine published version, assuming %j unpublished.", pkg2.name);
          return pkg2;
        }
      );
...
var require_get_tagged_packages = __commonJS({
  "libs/commands/publish/src/lib/get-tagged-packages.ts"(exports2, module2) {
    ...
    function getTaggedPackages(packageGraph, rootPath, execOpts) {
      ...
      return childProcess10.exec("git", ["diff-tree", "--name-only", "--no-commit-id", "--root", "-r", "-c", "HEAD"], execOpts).then(({ stdout }) => {
        const manifests = stdout.split("\n").filter((fp) => import_path18.default.basename(fp) === "package.json");
        const locations = new Set(manifests.map((fp) => import_path18.default.join(rootPath, import_path18.default.dirname(fp))));
  ...
  "libs/commands/publish/src/lib/get-packages-without-license.ts"(exports2, module2) {
    ...
    function getPackagesWithoutLicense(project, packagesToPublish) {
      return project.getPackageLicensePaths().then((licensePaths) => {
        const licensed = new Set(licensePaths.map((lp) => import_path18.default.dirname(lp)));
...
var require_git_checkout = __commonJS({
  "libs/commands/publish/src/lib/git-checkout.ts"(exports2, module2) {
    ...
    function gitCheckout(stagedFiles, gitOpts, execOpts) {
      ...
      return childProcess10.exec("git", ["checkout", "--"].concat(files), execOpts);
    }
  }
});
...
var require_remove_temp_licenses = __commonJS({
  "libs/commands/publish/src/lib/remove-temp-licenses.ts"(exports2, module2) {
    ...
    function removeTempLicenses(packagesToBeLicensed) {
      ...
      return (0, import_p_map5.default)(packagesToBeLicensed, (pkg2) => import_fs_extra11.default.remove(pkg2.licensePath));
    }
  }
});
    ...
    var PublishCommand = class extends Command {
      ...
      confirmPublish() {
        ...
        const message = this.packagesToPublish.map(
          (pkg2) => ` - ${pkg2.name} => ${this.updatesVersions.get(pkg2.name)}${pkg2.private ? " (private!)" : ""}`
        );
        output("");
      ...
      serializeChanges() {
        return (0, import_p_map5.default)(this.packagesToPublish, (pkg2) => pkg2.serialize());
      }
      resetChanges() {
        const { cwd } = this.execOpts;
        const gitOpts = {
        ...
        const dirtyManifests = [this.project.manifest].concat(this.packagesToPublish).map((pkg2) => import_path18.default.relative(cwd, pkg2.manifestLocation));
      ...
      execScript(pkg2, script) {
        const scriptLocation = import_path18.default.join(pkg2.location, "scripts", script);
        try {
        ...
        const mapper = (0, import_p_pipe.default)(
          ...[
            ...
            (pkg2) => pulseTillDone(packDirectory(pkg2, pkg2.location, opts)).then((packed) => {
              tracker.verbose("packed", import_path18.default.relative(this.project.rootPath, pkg2.contents));
              tracker.completeWork(1);
              pkg2.packed = packed;
              return pkg2.refresh();
            })
  ...
  "libs/commands/run/src/index.ts"(exports2, module2) {
    ...
    var RunCommand = class extends Command {
      ...
      initialize() {
        ...
        chain = chain.then(() => getFilteredPackages(this.packageGraph, this.execOpts, this.options));
        chain = chain.then((filteredPackages) => {
          this.packagesWithScript = script === "env" ? filteredPackages : filteredPackages.filter((pkg2) => {
            if (Array.isArray(this.script)) {
              return this.script.some((scriptName) => pkg2.scripts && pkg2.scripts[scriptName]);
            }
            return pkg2.scripts && pkg2.scripts[script];
          });
        });
        return chain.then(() => {
      ...
      execute() {
        ...
        return chain.then(() => {
          ...
          this.logger.success("", this.packagesWithScript.map((pkg2) => `- ${pkg2.name}`).join("\n"));
        });
      }
      getOpts(pkg2) {
        return {
      ...
      async runScriptsUsingNx() {
        ...
        if (this.options.profile) {
          ...
          process.env.NX_PROFILE = import_path18.default.relative(this.project.rootPath, absolutePath);
        }
      ...
      async prepNxOptions() {
        const nxJsonExists = (0, import_fs_extra11.existsSync)(import_path18.default.join(this.project.rootPath, "nx.json"));
      ...
      runScriptInPackagesParallel() {
        return (0, import_p_map5.default)(this.packagesWithScript, (pkg2) => this.runScriptInPackageStreaming(pkg2));
      }
...
init_src();
var changedCmd = __toESM(require_command());
var cleanCmd = __toESM(require_command2());
var diffCmd = __toESM(require_command3());
var execCmd = __toESM(require_command4());
var importCmd = __toESM(require_command5());
var infoCmd = __toESM(require_command6());
var initCmd = __toESM(require_command7());
var listCmd = __toESM(require_command8());
var publishCmd = __toESM(require_command10());
var runCmd = __toESM(require_command11());
var versionCmd = __toESM(require_command9());
var addCachingCmd = __toESM(require_command12());
var repairCmd = __toESM(require_command13());
var watchCmd = __toESM(require_command14());
var { addCmd, bootstrapCmd, linkCmd } = require("@lerna/legacy-package-management");
