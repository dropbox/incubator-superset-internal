...
var import_graceful_fs, import_is_stream, import_make_dir, import_path12, import_temp_dir, import_util, uuid, writeFileP, tempfile, writeStream, temp_write_default;
...
var import_node_fetch, import_npmlog34, import_path13, GitLabClient;
...
Request returned ${status} ${statusText}`);
      ...
      releasesUrl(namespace, project) {
        return new URL(
          `${this.baseUrl}/${import_path13.default.join("projects", encodeURIComponent(`${namespace}/${project}`), "releases")}`
        ).toString();
      }
    };
  }
...
var require_git_add = __commonJS({
  "libs/commands/version/src/lib/git-add.ts"(exports2, module2) {
    ...
    function resolvePrettier() {
      if (!resolvedPrettier) {
        try {
          const packageJson = (0, import_devkit2.readJsonFile)(import_path14.default.join(import_devkit2.workspaceRoot, "package.json"));
          const hasPrettier = packageJson.devDependencies?.prettier || packageJson.dependencies?.prettier;
          if (!hasPrettier) {
            return;
          }
          const prettierPath = import_path14.default.join(import_devkit2.workspaceRoot, "node_modules", "prettier");
    ...
    function maybeFormatFile(filePath) {
      ...
      const ignorePath = import_path14.default.join(import_devkit2.workspaceRoot, ".prettierignore");
      const fullFilePath = import_path14.default.join(import_devkit2.workspaceRoot, filePath);
    ...
    function gitAdd(changedFiles, gitOpts, execOpts) {
      ...
      for (const file of changedFiles) {
        const filePath = (0, import_slash2.default)(import_path14.default.relative(execOpts.cwd, import_path14.default.resolve(execOpts.cwd, file)));
...
var require_update_lockfile_version = __commonJS({
  "libs/commands/version/src/lib/update-lockfile-version.ts"(exports2, module2) {
    ...
    function updateLockfileVersion(pkg) {
      const lockfilePath = import_path14.default.join(pkg.location, "package-lock.json");
      let chain = Promise.resolve();
      chain = chain.then(
    ...
    var VersionCommand = class extends Command {
      ...
      updatePackageVersions() {
        ...
        if (this.options.npmClient === "pnpm") {
          chain = chain.then(() => {
            ...
            return childProcess10.exec("pnpm", ["install", "--lockfile-only", "--ignore-scripts", ...npmClientArgs], this.execOpts).then(() => {
              const lockfilePath = import_path14.default.join(this.project.rootPath, "pnpm-lock.yaml");
              changedFiles.add(lockfilePath);
            });
          });
        }
          ...
          chain = chain.then(() => childProcess10.execSync("yarn", ["--version"], this.execOpts)).then((yarnVersion) => {
            ...
            if (import_semver5.default.gte(yarnVersion, "2.0.0")) {
              ...
              return childProcess10.exec("yarn", ["install", "--mode", "update-lockfile", ...npmClientArgs], {
                ...
                const lockfilePath = import_path14.default.join(this.project.rootPath, "yarn.lock");
                changedFiles.add(lockfilePath);
              });
            }
          });
        ...
        if (this.options.npmClient === "npm" || !this.options.npmClient) {
          const lockfilePath = import_path14.default.join(this.project.rootPath, "package-lock.json");
