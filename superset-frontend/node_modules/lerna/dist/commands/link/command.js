...
function createPosixSymlink(src, dest, _type) {
  ...
  const relativeSymlink = import_path7.default.relative(import_path7.default.dirname(dest), src);
  if (_type === "exec") {
...
function shShim(src, dest, type) {
  ...
  const absTarget = import_path7.default.resolve(import_path7.default.dirname(dest), src);
  const scriptLines = ["#!/bin/sh", `chmod +x ${absTarget} && exec ${absTarget} "$@"`];
  return import_fs_extra3.default.writeFile(dest, scriptLines.join("\n"));
}
var import_cmd_shim, import_fs_extra3, import_npmlog18, import_path7;
  ...
  return Promise.all([Package.lazy(srcPackageRef), Package.lazy(destPackageRef)]).then(
    ...
    ([srcPackage, destPackage]) => {
      const actions = Object.keys(srcPackage.bin).map((name) => {
        const srcLocation = srcPackage.contents ? import_path12.default.resolve(srcPackage.location, srcPackage.contents) : srcPackage.location;
        const src = import_path12.default.join(srcLocation, srcPackage.bin[name]);
        const dst = import_path12.default.join(destPackage.binLocation, name);
        return { src, dst };
      });
      if (actions.length === 0) {
        return Promise.resolve();
...
var import_fs_extra8, import_p_map3, import_path12;
...
function resolveSymbolicLink(filePath) {
  ...
  const resolvedPath = lstat.isSymbolicLink() ? import_path13.default.resolve(import_path13.default.dirname(filePath), import_fs_extra9.default.readlinkSync(filePath)) : false;
  return {
    resolvedPath,
    lstat
  };
...
function resolveWindowsSymlink(filePath) {
  ...
  if (lstat.isFile() && !resolvedPath) {
    try {
      return import_path13.default.resolve(import_path13.default.dirname(filePath), readCmdShim.sync(filePath));
    } catch (e) {
      return false;
    }
  }
  return resolvedPath && import_path13.default.resolve(resolvedPath);
}
var import_fs_extra9, import_npmlog36, import_path13, readCmdShim;
...
function symlinkDependencies(packages, packageGraph, tracker) {
  ...
  return (0, import_p_map_series.default)(nodes, (currentNode) => {
    ...
    return (0, import_p_map4.default)(currentNode?.localDependencies ?? /* @__PURE__ */ new Map(), ([dependencyName, resolved]) => {
      ...
      const targetDirectory = import_path14.default.join(currentNodeModules ?? "", dependencyName);
      let chain = Promise.resolve();
      ...
      chain = chain.then((dirExists) => {
        if (dirExists) {
          ...
          return import_fs_extra10.default.ensureDir(import_path14.default.dirname(targetDirectory));
        }
      });
      const dependencyLocation = dependencyNode?.pkg.contents ? import_path14.default.resolve(dependencyNode.location, dependencyNode.pkg.contents) : dependencyNode?.location;
...
var import_fs_extra10, import_p_map4, import_p_map_series, import_path14;
...
var require_src = __commonJS({
  "libs/commands/link/src/index.ts"(exports2, module2) {
    "use strict";
    init_src();
        ...
        if (this.targetGraph) {
          for (const targetNode of this.targetGraph.values()) {
            ...
            for (const depNode of targetNode.localDependents.values()) {
              const depVersion = (0, import_slash2.default)(import_path15.default.relative(depNode.pkg.location, targetNode.pkg.location));
...
var require_command = __commonJS({
  "libs/commands/link/src/command.ts"(exports2, module2) {
    "use strict";
