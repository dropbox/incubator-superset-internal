...
class SliceModelView(SupersetModelView, DeleteMixin):  # noqa
    ...
    can_add = False
    label_columns = {'datasource_link': _('Datasource')}
    search_columns = (
        'slice_name',
        'description',
        'viz_type',
...
class Superset(BaseSupersetView):
    ...
    def get_raw_results(self, viz_obj):
        return self.json_response({'data': viz_obj.get_df().to_dict('records')})
    ...
    def save_or_overwrite_slice(
        ...
        if request.args.get('add_to_dash') == 'existing':
            ...
            flash(
                ...
                'to it'.format(dash.dashboard_title, slc.slice_name),
                'info',
            )
    ...
    def tables(self, db_id, schema, substr, force_refresh='false'):
        ...
        table_options = [{'value': tn, 'label': tn} for tn in table_names[:max_tables]]
        table_options.extend(
            [
                {'value': vn, 'label': '[view] {}'.format(vn)}
    ...
    def created_dashboards(self, user_id):
        ...
        qry = (
            ...
            .filter(
                sqla.or_(Dash.created_by_fk == user_id, Dash.changed_by_fk == user_id)
            )
    ...
    def created_slices(self, user_id=None):
        ...
        qry = (
            ...
            .filter(
                sqla.or_(Slice.created_by_fk == user_id, Slice.changed_by_fk == user_id)
            )
    ...
    def queries(self, last_updated_ms):
        ...
        queries_to_timeout = [
            client_id
            for client_id, query_dict in dict_queries.items()
            if (
                query_dict['state'] in unfinished_states
                and (
